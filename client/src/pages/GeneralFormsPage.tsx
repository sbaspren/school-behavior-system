...
const FORM_CATEGORIES: FormCategory[] = [
  { title: 'المواظبة والغياب', icon: 'event_busy', color: '#dc2626', forms: [
    { id: 'ghiab_bidon_ozr', title: 'متابعة غياب (بدون عذر)', desc: 'سجل تدرج الإجراءات للملف', icon: 'block', color: '#dc2626', requiresStudent: true },
  ...
  { title: 'نماذج الإجراءات السلوكية', icon: 'balance', color: '#4f46e5', forms: [
    { id: 'iltizam_madrasi', title: 'عقد التزام مدرسي', desc: 'يوقع بداية العام أو عند العودة', icon: 'handshake', color: '#4f46e5', requiresStudent: true },
    { id: 'tawid_darajat', title: 'فرص تعويض الدرجات', desc: 'لتحسين الدرجات المحسومة', icon: 'trending_up', color: '#7c3aed', requiresStudent: true, requiresViolation: true },
    ...
    { id: 'rasd_slooki', title: 'سجل مخالفات (للملف)', desc: 'يوضع في ملف الطالب', icon: 'folder', color: '#64748b', requiresStudent: true },
    { id: 'khota_tadeel', title: 'خطة تعديل سلوك', desc: 'للموجه الطلابي', icon: 'psychology', color: '#0d9488', requiresStudent: false, hasSpecialModal: true },
  ...
  { title: 'نماذج فارغة للتوزيع', icon: 'description', color: '#7c3aed', forms: [
    { id: 'rasd_moalem', title: 'سجل متابعة معلم', desc: 'طباعة فارغ أو مملوء', icon: 'school', color: '#7c3aed', requiresStudent: false, hasSpecialModal: true },
  ]},
  { title: 'النماذج السرية والطارئة', icon: 'emergency', color: '#b91c1c', forms: [
    { id: 'mashajara', title: 'محضر إثبات واقعة (مشاجرة)', desc: 'واقعة سلوك غير تربوي', icon: 'bolt', color: '#b91c1c', requiresStudent: false, hasSpecialModal: true },
    { id: 'eblagh_etha', title: 'إبلاغ عن إيذاء', desc: 'خاص بمركز البلاغات 1919', icon: 'shield', color: '#b91c1c', requiresStudent: false, hasSpecialModal: true },
    { id: 'high_risk', title: 'حالة عالية الخطورة', desc: 'للحالات الطارئة جداً', icon: 'emergency', color: '#7f1d1d', requiresStudent: false, hasSpecialModal: true },
  ]},
];
            ...
            {cat.forms.map(form => (
              <div key={form.id} onClick={() => openFormModal(form)} style={{ background: '#fff', padding: '20px', borderRadius: '16px', cursor: 'pointer', border: '2px solid #e5e7eb', transition: 'all 0.2s', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
                  <div style={{ width: '40px', height: '40px', background: `${form.color}14`, borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span className="material-symbols-outlined" style={{ fontSize: '22px', color: form.color }}>{form.icon}</span></div>
      ...
      {modalOpen && selectedForm && <div style={OV} onClick={() => setModalOpen(false)}><div style={{ background: '#fff', borderRadius: '20px', boxShadow: '0 25px 50px rgba(0,0,0,0.25)', width: '100%', maxWidth: '480px', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {selectedForm.requiresViolation && modalStudent && <div style={{ borderTop: '2px solid #e5e7eb', paddingTop: '12px' }}>
            <label style={{ fontSize: '12px', fontWeight: 700, color: '#374151', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}><span className="material-symbols-outlined" style={{ fontSize: 16, color: '#f59e0b' }}>warning</span> اختر المخالفة</label>
