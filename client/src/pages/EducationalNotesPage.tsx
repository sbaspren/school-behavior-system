...
// ────────────────────────── TODAY TAB ──────────────────────────
const TodayTab: React.FC<{ stage: string; noteTypes: string[]; onRefresh: () => void; schoolSettings: Record<string, string>; onTypesUpdated: (types: string[]) => void }> = ({ stage, noteTypes, onRefresh, schoolSettings, onTypesUpdated }) => {
              ...
              {detailStudent.notes.map(n => (
                <div key={n.id} style={{ border: '1px solid #e5e7eb', borderRadius: '12px', padding: '16px', marginBottom: '12px', borderRight: `4px solid ${SECTION_THEMES.notes}` }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <button onClick={() => { printForm('tawtheeq_tawasol', { studentName: detailStudent.name, grade: detailStudent.grade + ' / ' + detailStudent.cls, contactType: 'ملاحظة تربوية', contactReason: n.noteType + (n.details ? ' - ' + n.details : ''), violationDate: n.hijriDate || '', contactResult: n.isSent ? 'تم التواصل عبر الواتساب' : 'لم يتم الإرسال بعد', notes: 'المسجّل: ' + (n.teacherName || '-') }, schoolSettings as any); }} style={{ padding: '2px 8px', background: '#f0fdfa', color: '#0d9488', border: '1px solid #99f6e4', borderRadius: '8px', fontSize: '11px', fontWeight: 700, cursor: 'pointer' }}><span className="material-symbols-outlined" style={{fontSize:16,verticalAlign:'middle'}}>contact_phone</span> توثيق تواصل</button>
                  </div>
                </div>
              ))}
            </div>
