...
type TabType = 'today' | 'approved' | 'positive' | 'compensation';
...
const ViolationsPage: React.FC = () => {
  ...
  // Stats — todayViolations includes miladiDate check for backward compat
  ...
  return (
    <div className="sec-violations">
      ...
      {activeTab === 'approved' && (
        <ApprovedTab violations={filteredByStage} onRefresh={refresh} schoolSettings={schoolSettings} />
      )}
      {activeTab === 'positive' && (
...
const TodayTab: React.FC<{
  ...
  const handlePrintToday = () => {
    ...
    sorted.forEach((v, i) => {
      const key = `${v.grade} / ${classToLetter(v.className)}`;
  ...
  return (
    <>
      ...
      <ActionBar
        sectionColor="#4f46e5"
        leftButtons={[
          { icon: 'add_circle', label: 'تسجيل مخالفة', variant: 'primary', onClick: onAdd },
...
const ApprovedTab: React.FC<{
  ...
  const handlePrintArchive = () => {
    ...
    allFilteredRecords.forEach((r, i) => {
      ...
      const classKey = `${r.grade} (${classToLetter(r.className)})`;
    ...
    printListReport({
      ...
      headers: [
        { label: 'م', width: '5%' }, { label: 'الطالب', width: '20%' }, { label: 'الصف', width: '12%' },
              ...
              <div key={student.studentId} onClick={() => setDetailStudent({ studentId: student.studentId, studentName: student.studentName })}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '13px', color: '#6b7280' }}>{student.grade} ({classToLetter(student.className)})</div>
                  </div>
      ...
      {bulkSelected.size > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#1f2937', color: '#fff', padding: '12px 24px', zIndex: 40, boxShadow: '0 -4px 20px rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button onClick={() => setBulkSelected(new Set())} style={{ padding: '6px 12px', background: '#374151', color: '#d1d5db', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '13px' }}><span className="material-symbols-outlined" style={{ fontSize: '18px' }}>close</span></button>
          </div>
        </div>
      )}
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', background: '#f3f4f6', borderRadius: '8px', padding: '2px' }}>
          <button onClick={() => setViewMode('cards')} style={{ padding: '6px 12px', borderRadius: '6px', border: 'none', cursor: 'pointer', background: viewMode === 'cards' ? '#fff' : 'transparent', color: viewMode === 'cards' ? '#059669' : '#6b7280', fontWeight: 700, fontSize: '13px' }}><span className="material-symbols-outlined" style={{ fontSize: '16px', verticalAlign: 'middle' }}>view_carousel</span> بطاقات</button>
        ...
        <button onClick={() => {
          ...
          sorted.forEach((r, i) => {
            const key = `${r.grade} / ${classToLetter(r.className)}`;
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,0.6)', backdropFilter: 'blur(4px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: '#fff', borderRadius: '20px', boxShadow: '0 25px 50px rgba(0,0,0,0.25)', width: '100%', maxWidth: '700px', maxHeight: '85vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '16px 24px', background: 'linear-gradient(135deg, #d1fae5, #a7f3d0)', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 700 }}><span className="material-symbols-outlined" style={{ fontSize: '20px', verticalAlign: 'middle' }}>star</span> {detailStudent.name}</h3>
                <div style={{ fontSize: '14px', color: '#065f46', marginTop: '4px' }}>{detailStudent.records.length} سلوك متمايز</div>
              </div>
              <button onClick={() => setDetailStudent(null)} style={{ padding: '8px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '18px', color: '#6b7280' }}><span className="material-symbols-outlined" style={{ fontSize: '18px' }}>close</span></button>
            </div>
            <div style={{ padding: '16px 24px', overflowY: 'auto', flex: 1 }}>
              <table className="data-table">
                ...
                {filtered.map((v, i) => {
                  ...
                  return (
                    <tr key={v.id}>
                      ...
                      <td>{v.grade} ({classToLetter(v.className)})</td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,0.6)', backdropFilter: 'blur(4px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: '#fff', borderRadius: '20px', boxShadow: '0 25px 50px rgba(0,0,0,0.25)', width: '100%', maxWidth: '500px', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', background: 'linear-gradient(135deg, #d1fae5, #a7f3d0)', display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <button onClick={() => setCompensateModal(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '20px', color: '#065f46' }}><span className="material-symbols-outlined" style={{ fontSize: '20px' }}>close</span></button>
            </div>
            ...
            <div style={{ padding: '16px 20px', background: '#f9fafb', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              ...
              }}>{saving ? 'جاري الحفظ...' : <><span className="material-symbols-outlined" style={{ fontSize: '16px', verticalAlign: 'middle' }}>check</span> تسجيل التعويض</>}</button>
            </div>
          </div>
        </div>
      )}
...
const ReportsTab: React.FC<{
  ...
  const topStudents = useMemo(() => {
    ...
    for (const v of filteredViols) {
      const g = groups.get(v.studentId) || { name: v.studentName, grade: v.grade, cls: classToLetter(v.className), count: 0, deduction: 0 };
      g.count++;
  ...
  const handlePrintReport = () => {
    ...
    // قسم 1: التوزيع حسب الدرجة — 3 أعمدة
    const degreeRows: ListReportRow[] = [];
    byDegree.forEach(d => degreeRows.push({ cells: [d.label, toIndic(d.count), toIndic(d.deduction)] }));

    // قسم 2: التوزيع حسب الفصل — 2 أعمدة
    const classRows: ListReportRow[] = [];
    byClass.slice(0, 15).forEach(c => classRows.push({ cells: [c.name, toIndic(c.count)] }));

    // قسم 3: أكثر الطلاب مخالفات — 6 أعمدة
    const studentRows: ListReportRow[] = [];
    topStudents.forEach((s, i) => studentRows.push({ cells: [toIndic(i + 1), `<strong>${s.name}</strong>`, `${s.grade} (${classToLetter(s.cls)})`, toIndic(s.count), toIndic(s.deduction), toIndic(s.score)] }));

    printListReport({
      title: 'تقرير المخالفات السلوكية',
      dateText: hijri,
      statsBar: `إجمالي المخالفات: ${toIndic(filteredViols.length)} | إجمالي الحسم: ${toIndic(totalDeduction)}`,
      headers: [], rows: [], // لا تُستخدم مع sections
      sections: [
        {
          title: 'التوزيع حسب الدرجة',
          headers: [{ label: 'الدرجة', width: '50%' }, { label: 'عدد المخالفات', width: '25%' }, { label: 'إجمالي الحسم', width: '25%' }],
          rows: degreeRows,
        },
        {
          title: 'التوزيع حسب الفصل',
          headers: [{ label: 'الفصل', width: '60%' }, { label: 'عدد المخالفات', width: '40%' }],
          rows: classRows,
        },
        {
          title: 'أكثر الطلاب مخالفات',
          headers: [
            { label: 'م', width: '6%' }, { label: 'الطالب', width: '34%' }, { label: 'الصف', width: '15%' },
            { label: 'المخالفات', width: '15%' }, { label: 'الحسم', width: '15%' }, { label: 'درجة السلوك', width: '15%' },
          ],
          rows: studentRows,
        },
      ],
              ...
              {topStudents.map((s, i) => {
                ...
                return (
                  <tr key={s.id}>
                    ...
                    <td>{s.grade} ({classToLetter(s.cls)})</td>
...
const DawatModal: React.FC<{ violation: ViolationRow; onClose: () => void; schoolSettings: Record<string, string> }> = ({ violation, onClose, schoolSettings }) => {
  ...
  const handlePrint = () => {
    ...
    const data: PrintFormData = {
      ...
      grade: `${violation.grade} / ${classToLetter(violation.className)}`,
      ...
      <div style={{ background: '#fff', borderRadius: '20px', boxShadow: '0 25px 50px rgba(0,0,0,0.25)', width: '100%', maxWidth: '480px', overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', background: 'linear-gradient(135deg, #f59e0b, #d97706)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: '12px' }}>{violation.studentName} — {violation.grade}/{classToLetter(violation.className)}</div>
          </div>
        ...
        <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '13px', color: '#059669', fontWeight: 700, marginTop: '4px', textAlign: 'center' }}>{formatHijriDate(date)}</div>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div>
          ...
          {selectedStudent ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: '#f0fdf4', borderRadius: '8px', border: '1px solid #bbf7d0' }}>
              <div>
                ...
                <span style={{ fontSize: '13px', color: '#6b7280', marginRight: '8px' }}>{selectedStudent.grade} ({classToLetter(selectedStudent.className)})</span>
              </div>
