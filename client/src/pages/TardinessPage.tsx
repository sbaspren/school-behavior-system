...
const TodayTab: React.FC<{ records: TardinessRow[]; allRecords: TardinessRow[]; onRefresh: () => void; stageFilter: string; schoolSettings: Record<string, string>; onAdd: () => void }> = ({ records, allRecords, onRefresh, stageFilter, schoolSettings, onAdd }) => {
  ...
  return (
    <>
      ...
      {selected.size > 0 && (
        <div style={{ position: 'fixed', bottom: 16, left: '50%', transform: 'translateX(-50%)', background: 'rgba(30,64,175,.95)', color: '#fff', padding: '12px 24px', borderRadius: 100, boxShadow: '0 8px 32px rgba(0,0,0,.3)', display: 'flex', alignItems: 'center', gap: 16, zIndex: 50, backdropFilter: 'blur(8px)' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ fontWeight: 800, fontSize: 16 }}>{selected.size}</span><span style={{ fontSize: 13 }}>محدد</span></span>
          <span style={{ width: 1, height: 24, background: 'rgba(255,255,255,.3)' }} />
      ...
      {msgEditorRow && <MessageEditorModal record={msgEditorRow} onSend={(msg) => handleConfirmSend(msgEditorRow, msg)} onClose={() => setMsgEditorRow(null)} />}
    </>
  );
};

// ============================== Message Editor Modal ==============================
const MessageEditorModal: React.FC<{ record: TardinessRow; onSend: (message: string) => void; onClose: () => void }> = ({ record, onSend, onClose }) => {
  ...
  useEffect(() => { templatesApi.getByType('تأخر').then(res => { const saved = res.data?.data?.template; if (saved) { setMessage(saved.replace('{اسم_الطالب}', record.studentName).replace('{نوع_التأخر}', typeLabel).replace('{الحصة}', record.period || '').replace('{التاريخ}', hijriDate)); setTemplateLoaded(true); } }).catch(() => {}); }, []);
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,.6)', backdropFilter: 'blur(4px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 25px 50px rgba(0,0,0,.25)', width: '100%', maxWidth: 520, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={8} style={{ width: '100%', padding: 12, border: '2px solid #d1d5db', borderRadius: 12, fontSize: 14, lineHeight: 1.8, resize: 'vertical', boxSizing: 'border-box', direction: 'rtl' }} />
        ...
        <div style={{ padding: '16px 24px', background: '#f9fafb', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
          ...
          <button onClick={() => onSend(message)} style={{ padding: '8px 24px', background: '#25d366', color: '#fff', borderRadius: 8, fontWeight: 700, border: 'none', cursor: 'pointer' }}><span className="material-symbols-outlined" style={{ fontSize: 16, verticalAlign: 'middle' }}>smartphone</span> إرسال</button>
        </div>
      </div>
    </div>
  );
...
const ApprovedTab: React.FC<{ records: TardinessRow[]; onRefresh: () => void; schoolSettings: Record<string, string>; stageFilter: string }> = ({ records, onRefresh, schoolSettings, stageFilter }) => {
  ...
  const handlePrintContactReport = () => {
    ...
    const rows: ListReportRow[] = sent.map((r, i) => ({ cells: [toIndic(i+1), `<span style="font-weight:bold">${escapeHtml(r.studentName)}</span>`, escapeHtml(r.grade+' / '+r.className), escapeHtml(TARDINESS_TYPES[r.tardinessType]?.label||r.tardinessType), escapeHtml(r.hijriDate||''), '<span style="color:green;font-weight:bold">تم</span>'] }));
    printListReport({ title: 'تقرير التواصل مع أولياء الأمور', subtitle: 'التأخر' + (stageFilter !== '__all__' ? ' - ' + stageFilter : ''), dateText: hijri + ' | عدد الطلاب: ' + toIndic(sent.length), headers: [{ label: 'م', width: '5%' }, { label: 'اسم الطالب', width: '28%' }, { label: 'الصف', width: '12%' }, { label: 'نوع التأخر', width: '15%' }, { label: 'التاريخ', width: '15%' }, { label: 'التواصل', width: '10%' }], rows, summary: toIndic(sent.length) + ' سجل' }, schoolSettings as any);
  };
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        <button onClick={handlePrintContactReport} style={{ height: 34, padding: '0 12px', background: '#059669', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 700, cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}><span className="material-symbols-outlined" style={{ fontSize: 16 }}>contact_phone</span> تقرير التواصل</button>
      </div>
      ...
      <div style={{ background: '#fff', padding: 12, borderRadius: 12, border: '1px solid #e5e7eb', marginBottom: 12 }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="ابحث بالاسم..." style={{ width: 180, height: 34, padding: '0 10px', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 13 }} />
          ...
          <div style={{ display: 'flex', gap: 2, background: '#f3f4f6', borderRadius: 8, padding: 2 }}>
            <button onClick={() => setViewMode('cards')} style={{ padding: '4px 8px', borderRadius: 6, border: 'none', cursor: 'pointer', background: viewMode === 'cards' ? '#fff' : 'transparent', color: viewMode === 'cards' ? '#dc2626' : '#9ca3af' }}><span className="material-symbols-outlined" style={{ fontSize: 18 }}>grid_view</span></button>
            <button onClick={() => setViewMode('table')} style={{ padding: '4px 8px', borderRadius: 6, border: 'none', cursor: 'pointer', background: viewMode === 'table' ? '#fff' : 'transparent', color: viewMode === 'table' ? '#dc2626' : '#9ca3af' }}><span className="material-symbols-outlined" style={{ fontSize: 18 }}>table_rows</span></button>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, marginTop: 8, paddingTop: 8, borderTop: '1px solid #f3f4f6', alignItems: 'center', flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: '#9ca3af' }}>فترة:</span>
          <input type="text" value={dateFrom} onChange={e => setDateFrom(e.target.value)} placeholder="من تاريخ (هجري)" style={{ height: 30, padding: '0 8px', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 12, width: 130 }} />
          <span style={{ fontSize: 12, color: '#9ca3af' }}>إلى</span>
          <input type="text" value={dateTo} onChange={e => setDateTo(e.target.value)} placeholder="إلى تاريخ (هجري)" style={{ height: 30, padding: '0 8px', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 12, width: 130 }} />
          {(dateFrom || dateTo) && <button onClick={() => { setDateFrom(''); setDateTo(''); }} style={{ padding: '2px 8px', background: 'none', color: '#9ca3af', border: 'none', cursor: 'pointer', fontSize: 11 }}><span className="material-symbols-outlined" style={{ fontSize: 14, verticalAlign: 'middle' }}>clear</span> مسح</button>}
...
const StudentDetailModal: React.FC<{ studentName: string; records: TardinessRow[]; onClose: () => void; onRefresh: () => void; schoolSettings: Record<string, string> }> = ({ studentName, records, onClose, onRefresh, schoolSettings }) => {
  ...
  const handlePrint = () => {
    const hijri = new Date().toLocaleDateString('ar-SA-u-ca-islamic-umalqura', { day: 'numeric', month: 'long', year: 'numeric' });
    const rows: ListReportRow[] = records.map((r, i) => ({ cells: [toIndic(i+1), escapeHtml(r.hijriDate||''), escapeHtml(TARDINESS_TYPES[r.tardinessType]?.label||r.tardinessType), toIndic(r.period||'-'), r.isSent ? '<span style="color:green;font-weight:bold">✓</span>' : '<span style="color:#ea580c">✗</span>'] }));
    printListReport({ title: 'سجل التأخر', subtitle: studentName, dateText: hijri, statsBar: `إجمالي: ${toIndic(records.length)} | صباحي: ${toIndic(records.filter(r=>r.tardinessType==='Morning').length)} | حصة: ${toIndic(records.filter(r=>r.tardinessType==='Period').length)}`, headers: [{ label: 'م', width: '8%' }, { label: 'التاريخ', width: '25%' }, { label: 'النوع', width: '25%' }, { label: 'الحصة', width: '20%' }, { label: 'الإرسال', width: '12%' }], rows, summary: `إجمالي: ${toIndic(records.length)} تأخر` }, schoolSettings as any);
  };
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,.6)', backdropFilter: 'blur(4px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 25px 50px rgba(0,0,0,.25)', width: '100%', maxWidth: 700, maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '12px 24px', background: '#f9fafb', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handleSendAll} style={{ padding: '6px 12px', background: '#25d366', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 700, cursor: 'pointer', fontSize: 12 }}><span className="material-symbols-outlined" style={{ fontSize: 16, verticalAlign: 'middle' }}>smartphone</span> إرسال الكل</button>
            <button onClick={handlePrint} style={{ padding: '6px 12px', background: '#4f46e5', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 700, cursor: 'pointer', fontSize: 12 }}><span className="material-symbols-outlined" style={{ fontSize: 16, verticalAlign: 'middle' }}>print</span> طباعة</button>
          </div>
