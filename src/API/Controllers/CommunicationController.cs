...
public class CommunicationController : ControllerBase
{
        ...
        [FromQuery] int? studentId = null)
    {
        ...
        {
            // ★ يتعامل مع القيم القديمة: "sent"→"تم", "failed"→"فشل"
            if (status == "\u062a\u0645") // تم
                query = query.Where(r => r.SendStatus == "\u062a\u0645" || r.SendStatus == "sent" || r.SendStatus.Contains("\u062a\u0645 \u0627\u0644\u0625\u0631\u0633\u0627\u0644"));
            else if (status == "\u0641\u0634\u0644") // فشل
                query = query.Where(r => r.SendStatus == "\u0641\u0634\u0644" || r.SendStatus == "failed" || r.SendStatus.Contains("\u0641\u0634\u0644"));
            else
        ...
        [FromQuery] string? stage = null)
    {
        ...
        // ★ يتعامل مع جميع أشكال الحالة: "تم" / "sent" / "✅ تم الإرسال"
        var sent = await query.CountAsync(r =>
            r.SendStatus == "\u062a\u0645" || r.SendStatus == "sent" ||
            r.SendStatus.Contains("\u062a\u0645 \u0627\u0644\u0625\u0631\u0633\u0627\u0644")); // تم / sent / تم الإرسال
        var failed = await query.CountAsync(r =>
