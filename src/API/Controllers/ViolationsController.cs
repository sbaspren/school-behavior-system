...
public class ViolationsController : RecordControllerBase
{
    public ViolationsController(AppDbContext db, IWhatsAppServerService wa, IHijriDateService hijri, ISemesterService semesterSvc, IMemoryCache cache)
        ...
        [FromQuery] bool? isSent = null)
    {
        var query = ApplyCommonFilters(
            Db.Violations.AsQueryable(),
            stage, grade, className, studentId, dateFrom, dateTo, isSent);

        // Entity-specific filters
        if (degree.HasValue)
            query = query.Where(v => (int)v.Degree == degree.Value);

        // Search includes Description (entity-specific), so applied separately
    ...
    {
        var baseQuery = ApplyStageFilter(Db.Violations.AsQueryable(), stage);
        ...
        [FromQuery] int studentId, [FromQuery] int degree, [FromQuery] string? violationCode = null)
    {
        ...
        if (!string.IsNullOrEmpty(violationCode))
            query = Db.Violations.Where(v => v.StudentId == studentId && v.ViolationCode == violationCode);
        else
            query = Db.Violations.Where(v => v.StudentId == studentId && (int)v.Degree == degree);
    ...
    public async Task<ActionResult<ApiResponse>> AddViolation([FromBody] ViolationRequest request)
    {
        ...
        {
            var violDef = await Db.ViolationTypeDefs
        ...
        var currentCount = !string.IsNullOrEmpty(request.ViolationCode)
            ? await Db.Violations.CountAsync(v => v.StudentId == request.StudentId && v.ViolationCode == request.ViolationCode)
            : await Db.Violations.CountAsync(v => v.StudentId == request.StudentId && v.Degree == degree);
        ...
        var dayName = Hijri.GetHijriDateFull().WeekdayAr;
        ...
        await StampSemesterAsync(violation);
    ...
    public async Task<ActionResult<ApiResponse<object>>> AddViolationsBatch([FromBody] ViolationBatchRequest request)
    {
        ...
        var existingCounts = await Db.Violations
        ...
        // التاريخ الهجري
        var hijriDate = Hijri.GetHijriDate();
        ...
        foreach (var student in students)
        {
            ...
            var batchViolation = new Violation
            {
            ...
            await StampSemesterAsync(batchViolation);
            violations.Add(batchViolation);
    ...
    public async Task<ActionResult<ApiResponse<object>>> GetCompensationEligible(
        [FromQuery] string? stage = null)
    {
        var query = ApplyStageFilter(Db.Violations.Where(v => v.Deduction > 0), stage);
        ...
        var compensatedIdsList = await Db.PositiveBehaviors
        ...
        var compensationCountsByStudent = await Db.PositiveBehaviors
    ...
    {
        return await UpdateSentStatusAsync(Db.Violations, id, request.IsSent, "المخالفة غير موجودة");
    }

    // تحديث حالة الإرسال جماعي (updateViolationSentStatus)
    [HttpPut("sent-batch")]
    public async Task<ActionResult<ApiResponse<object>>> UpdateSentBatch([FromBody] CommonBulkIdsRequest request)
    {
        return await UpdateSentBatchAsync(Db.Violations, request.Ids, "لا توجد مخالفات محددة");
    }
    ...
    {
        var violation = await Db.Violations.Include(v => v.Student).FirstOrDefaultAsync(v => v.Id == id);
        if (violation == null)
        ...
        return await SendWhatsAppSingleAsync(
            violation, phone, message,
            request.SenderPhone,
            "مخالفة سلوكية",
            $"مخالفة درجة {(int)violation.Degree}",
            request.SentBy ?? "");
    }
    ...
    public async Task<ActionResult<ApiResponse<object>>> SendWhatsAppBulk([FromBody] CommonBulkSendWhatsAppRequest request)
    {
        ...
        var (sentCount, failedCount, total) = await SendWhatsAppBulkCoreAsync(
            violations,
            v => v.Student?.Mobile ?? "",
            v => Task.FromResult(BuildDefaultMessage(v)),
            request.SenderPhone,
            "مخالفة سلوكية",
            v => $"مخالفة درجة {(int)v.Degree}",
            request.SentBy ?? "",
            speed: request.Speed,
    ...
    {
        return await DeleteRecordAsync(Db.Violations, id, "المخالفة غير موجودة", "تم حذف المخالفة بنجاح");
    }
    ...
    {
        return await DeleteBulkAsync(Db.Violations, request.Ids, "لا توجد مخالفات محددة");
    }
    ...
    {
        var types = await Db.ViolationTypeDefs
            .OrderBy(t => t.Code)
            .Select(t => new
            {
    ...
    {
        var violations = await ApplyStageFilter(Db.Violations.AsQueryable(), stage)
