...
public class TeacherInputController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<object>>> GetClassStudents([FromQuery] string className, [FromQuery] string? token = null)
    {
        ...
        // ★ مطابقة: الصف يحتوي على اسم الصف + رقم الفصل يطابق (دعم الحرف والرقم)
        var numToLetter = new Dictionary<string, string>
        {
            ["1"] = "أ", ["2"] = "ب", ["3"] = "ج", ["4"] = "د",
            ["5"] = "ه", ["6"] = "و", ["7"] = "ز", ["8"] = "ح", ["9"] = "ط"
        };
        var letterToNum2 = new Dictionary<string, string>
        {
            ["أ"] = "1", ["ب"] = "2", ["ج"] = "3", ["د"] = "4",
            ["ه"] = "5", ["هـ"] = "5", ["و"] = "6", ["ز"] = "7", ["ح"] = "8", ["ط"] = "9"
        };
        var altSection = numToLetter.GetValueOrDefault(targetSection,
            letterToNum2.GetValueOrDefault(targetSection, targetSection));
        var students = await query
            .Where(s => s.Grade.Contains(targetGrade) && (s.Class == targetSection || s.Class == altSection))
            .OrderBy(s => s.Name)
            .Select(s => new
            {
                id = s.StudentNumber,
